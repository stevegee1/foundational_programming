    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(1, amount // price + 1):
                # Hint: The display function will help you add fruit to the
                # cart.
...
def distance(city_a, city_b):
    ...
    long_a = get_lon(city_a)
    lat_b = get_lat(city_b)
    long_b = get_lon(city_b)
    return (sqrt((lat_a - lat_b)**2 + (long_a - long_b)**2))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    cord = make_city('cord', lat, lon)
    dist_from_cord_to_city_a = distance(cord, city_a)
    dist_from_cord_to_city_b = distance(cord, city_b)
    if (dist_from_cord_to_city_a < dist_from_cord_to_city_b):
        return get_name(city_a)
    elif (dist_from_cord_to_city_b < dist_from_cord_to_city_a):
